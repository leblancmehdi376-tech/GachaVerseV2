...
describe('calculateCharacterEquippedDps', () => {
  it('renvoie 0 si le templateId ne correspond à aucun personnage connu', () => {
    ...
    expect(n(calculateCharacterEquippedDps(owned.templateId, owned, []))).toBe(0);
  });

  it('renvoie un DPS positif pour un personnage valide', () => {
    const owned = makeOwned(sampleTpl.id, { level: 10 });
    const dps = n(calculateCharacterEquippedDps(sampleTpl.id, owned, []));
...
describe('calculateEquippedTeamDps', () => {
  it('vaut 0 pour une équipe vide', () => {
    expect(n(calculateEquippedTeamDps([null, null, null], {}))).toBe(0);
  });
  ...
  it('ignore les emplacements vides et les personnages absents de la collection', () => {
    ...
    const dps = n(calculateEquippedTeamDps([sampleTpl.id, null, 'inconnu_dans_collection'], collection));
    expect(dps).toBeGreaterThan(0);
  });
  ...
  it("le DPS total croît quand on ajoute un deuxième personnage à l'équipe", () => {
    ...
    const oneChar  = n(calculateEquippedTeamDps([sampleTpl.id, null], collection));
    const twoChars = n(calculateEquippedTeamDps([sampleTpl.id, sampleTpl2.id], collection));
