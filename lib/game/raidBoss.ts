// ── Monarque des Ombres — Boss de raid ─────────────────────────────
...
export interface RaidBossDef {
  ...
  coinItemId: string;    // ItemDef de la pièce de raid (lib/game/items.ts)
...
// Table de drop partagée par tous les boss de raid : seuls les 3 objets
...
// principal ci-dessus) — voir RAID_TITLES dans lib/game/titles.ts.
function titleDropEntry(title: string): DropEntry {
  return { weight: 1.0, coinQty: 0, result: { type: 'title', id: title } };
}

export const SHADOW_MONARCH_BOSS: RaidBossDef = {
  id:          'shadow_monarch',
  name:        'Monarque des Ombres',
  subtitle:    'Le Roi qui se tient au sommet de tous les Monarques',
  spritePath:  '/sprites/raids/shadow_monarch.webp',
  bgImagePath: '/sprites/raids/shadow_monarch_bg',
  ...
  buyCost:     100,
  dropTable: [...buildRaidDropTable(['elixir_vie', 'manteau_ombre', 'beru']), titleDropEntry(RAID_TITLES.shadow_monarch)],
};

export const ARTHUR_LEYWIN_BOSS: RaidBossDef = {
  id:          'arthur_leywin',
  name:        'Arthur Leywin',
  subtitle:    'Héritier de l’Aube',
  spritePath:  '/sprites/raids/arthur_leywin.webp',
  bgImagePath: '/sprites/raids/arthur_leywin_bg',
  ...
  buyCost:     300,
  dropTable: [...buildRaidDropTable(['cristal_ether', 'epee_ether', 'sylvia']), titleDropEntry(RAID_TITLES.arthur_leywin)],
};

export const EMINENCE_SHADOW_BOSS: RaidBossDef = {
  id:          'eminence_shadow',
  name:        "L'Éminence de l'Ombre",
  subtitle:    'Celui qui se tapit dans l’ombre',
  spritePath:  '/sprites/raids/cid_kagenou.webp',
  bgImagePath: '/sprites/raids/cid_kagenou_bg',
...
// perso, voir buyRaidCharacter dans store/gameStore.ts).
export function getRaidCharacterCost(boss: RaidBossDef, purchases: number): number {
...
// contre le type du boss, ±10% cumulable par compagnon — voir RaidPage.tsx).
...
export function getRaidBossMaxHp(boss: RaidBossDef, currentPower: BigNum, durationMult: number = 1): BigNum {
