...
import { RAID_QUESTS } from '@/store/gameStoreHelpers';
 ...
 */
export async function resetPlayerRaidQuests(uid: string): Promise<boolean> {
  if (!db) return false;
  try {
    await updateDoc(doc(db, 'saves', uid), {
    ...
    logger.error('[AdminTools] resetPlayerRaidQuests:', e);
    return false;
  }
}
