      ...
      <div onClick={e => e.stopPropagation()} style={{ background: 'var(--c-panel)', border: '2px solid #8b5cf6', padding: '24px', width: '340px', display: 'flex', flexDirection: 'column', gap: '14px', boxShadow: '0 0 32px #8b5cf644' }}>
        ...
        <div style={{ display: 'flex', gap: '4px' }}>
          {(['signin','signup'] as const).map(m => (
            <button key={m} onClick={() => setMode(m)} style={{ flex: 1, padding: '8px', background: mode===m ? '#8b5cf622' : 'none', border: `1px solid ${mode===m ? '#8b5cf6' : 'var(--c-border)'}`, color: mode===m ? '#a78bfa' : 'var(--c-muted)', cursor: 'pointer', fontFamily: 'var(--font-pixel)', fontSize: '8.2px' }}>
        ...
        {mode === 'signin' ? (
          ...
          <>
            <input type="text" placeholder="Pseudo en jeu" value={username} onChange={e => setUsername(e.target.value)} onKeyDown={e => e.key==='Enter' && handleSubmit()} style={{ background: '#08090f', border: '1px solid var(--c-border)', color: 'var(--c-text)', padding: '10px 12px', fontFamily: 'var(--font-pixel)', fontSize: '9.3px', outline: 'none', borderRadius: '2px' }} />
            <input type="email" placeholder="email@example.com" value={email} onChange={e => setEmail(e.target.value)} onKeyDown={e => e.key==='Enter' && handleSubmit()} style={{ background: '#08090f', border: '1px solid var(--c-border)', color: 'var(--c-text)', padding: '10px 12px', fontFamily: 'var(--font-pixel)', fontSize: '9.3px', outline: 'none', borderRadius: '2px' }} />
            <input type="password" placeholder="Mot de passe" value={password} onChange={e => setPassword(e.target.value)} onKeyDown={e => e.key==='Enter' && handleSubmit()} style={{ background: '#08090f', border: '1px solid var(--c-border)', color: 'var(--c-text)', padding: '10px 12px', fontFamily: 'var(--font-pixel)', fontSize: '9.3px', outline: 'none', borderRadius: '2px' }} />
            <input type="text" placeholder="Discord (@nom ou nom#1234)" value={discordUsername} onChange={e => setDiscordUsername(e.target.value)} onKeyDown={e => e.key==='Enter' && handleSubmit()} style={{ background: '#08090f', border: '1px solid var(--c-border)', color: 'var(--c-text)', padding: '10px 12px', fontFamily: 'var(--font-pixel)', fontSize: '9.3px', outline: 'none', borderRadius: '2px' }} />
          </>
        )}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <span style={{ fontFamily: 'var(--font-pixel)', fontSize: '7.2px', color: 'var(--c-muted)' }}>OU</span>
          <div style={{ flex: 1, height: '1px', background: 'var(--c-border)' }} />
        </div>
        <button onClick={async () => { setLoading(true); try { await signInGoogle(); onClose(); } catch(e:unknown){setError(e instanceof Error?e.message:'Erreur');} finally{setLoading(false);} }}
          disabled={loading} style={{ padding: '10px', background: '#08090f', border: '1px solid var(--c-border)', color: 'var(--c-text)', cursor: 'pointer', fontFamily: 'var(--font-pixel)', fontSize: '8.2px' }}>
